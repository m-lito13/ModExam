...
- **CORS** — ProductApi has CORS enabled for the React app's origins (`http://localhost:5173`, `http://localhost:3000`), configured under `Cors:AllowedOrigins` in `appsettings.Development.json`.
- **Credentials in plain text (testing only)** — the SQL Server SA password (`Testpwd123!`) appears in plain text in `appsettings.json`/`appsettings.Development.json` and in `docker-compose.yml`/`docker-compose.apps.yml`. That's fine for this local/testing setup, but not how it should be done for anything real: a production setup should pull secrets from something like .NET User Secrets (local dev), environment variables kept out of source control, or a secrets manager (Key Vault, AWS Secrets Manager, Docker secrets) instead of committing them.
